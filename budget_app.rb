...
class Application
  ...
  def enter_debt
    ...
    # Stores the sum of all elements (dollar amounts), in the input array (input_arr) within a variable called total_cc_debt (Total credit card debt).
    @total_cc_debt = input_arr.sum

    # Enter total credit card debt amount.
...
#{(@total_cc_debt.to_f / @total_monthly_income.to_f).truncate(2)} months."
  ...
  def debt_exceeds_income
    # If/then then conditional statement, when debt exceeds the total monthly income amount.
